...
class SQLAnyDialect(default.DefaultDialect):
    ...
    def get_table_id(self, connection, table_name, schema=None, **kw):
        ...
        #                             schema_name=schema,
        #                             table_name=table_name) #sk
        result = connection.execute(TABLEID_SQL, {'schema_name':schema,'table_name':table_name} )
    ...
    def get_foreign_keys(self, connection, table_name, schema=None, **kw):
        ...
        results = connection.execute(COLUMN_SQL, {'table_id':table_id})
        #results = connection.execute(COLUMN_SQL, table_id=table_id)#sk
        
        columns = {}
        for col in results:
            columns[col[0]] = col[1] 
            # columns[col["id"]] = col["name"] #sk
        ...
        """)
        referential_constraints = connection.execute(REFCONSTRAINT_SQL,{'table_id':table_id})
        #referential_constraints = connection.execute(REFCONSTRAINT_SQL,table_id=table_id)#sk
        ...
        for r in referential_constraints:
            reftable_id = r[2] #reftable_id = r["reftable_id"] #sk
            foreign_index_id = r[0] # foreign_index_id = r["foreign_index_id"] #sk

            if reftable_id not in table_cache:
                # c = connection.execute(REFTABLE_SQL, table_id=reftable_id) #sk
                c = connection.execute(REFTABLE_SQL, {'table_id':reftable_id})
                reftable = c.fetchone()
                c.close()
                table_info = {"name": reftable[0], "schema": None}
                #table_info = {"name": reftable["name"], "schema": None}#sk
                if (schema is not None or
                        reftable["schema"] != self.default_schema_name):
                    table_info["schema"] = reftable[1]
                    #table_info["schema"] = reftable["schema"]#sk

                table_cache[reftable_id] = table_info
                results = connection.execute(COLUMN_SQL, {'table_id':reftable_id}) #sk
                reftable_columns = {}
                for col in results:
                    reftable_columns[col[0]] = col[1]
            ...
            """)
            # ref_cols = connection.execute(REFCOLS_SQL,table_id=table_id,reftable_id=reftable_id,foreign_index_id=foreign_index_id)#sk
            ref_cols = connection.execute(REFCOLS_SQL, {'table_id':table_id,'reftable_id':reftable_id,'foreign_index_id':foreign_index_id})
            for rc in ref_cols:
                constrained_columns.append(columns[rc[0]]) # constrained_columns.append(columns[rc["fokey"]])#sk
                referred_columns.append(reftable_columns[rc[1]]) # referred_columns.append(reftable_columns[rc["refkey"]])
            ...
            fk_info = {
                    ...
                    "name": r[1] #"name": r["name"] #sk line 614
                }
    ...
    def get_indexes(self, connection, table_name, schema=None, **kw):
        ...
        for r in results:
            ...
            """)
            idx_cols = connection.execute(INDEXCOL_SQL, {'index_id':r[0],'table_id':table_id}) #sk
            column_names = [ic[0] for ic in idx_cols] #sk
            index_info = {"name": r[1],
                          "unique": bool(r[2]),
    ...
    def get_pk_constraint(self, connection, table_name, schema=None, **kw):
        ...
        PKCOL_SQL = text("""
             ...
             WHERE ic.index_id = :index_id and ic.table_id = :table_id
            """)
        # pk_cols = connection.execute(PKCOL_SQL, index_id=pks["index_id"],table_id=table_id )
        pk_cols = connection.execute(PKCOL_SQL, {'index_id':pks[1],'table_id':table_id} )

        print("=======pk_cols line 754===============")
        for row1 in pk_cols:
             print(row1)

        column_names = [pkc["col"] for pkc in pk_cols]
        # return {"constrained_columns": column_names,"name": pks["name"]}
        return {"constrained_columns": column_names,"name": pks[2]}
    ...
    def get_unique_constraints(self, connection, table_name, schema=None, **kw):
        ...
        results = connection.execute(INDEX_SQL, {'table_id':table_id}) #sk
        indexes = []
        for r in results:
            INDEXCOL_SQL = text("""
             select tc.column_name as col
            ...
            idx_cols = connection.execute(INDEXCOL_SQL, { 'index_id': r[0],'table_id':table_id })#sk
            column_names = [ic[0] for ic in idx_cols]#sk
            index_info = {"name": r[1],"column_names": column_names}#sk
            indexes.append(index_info)
    ...
    def get_table_names(self, connection, schema=None, **kw):
        ...
        TABLE_SQL = text("""
          ...
          FROM sys.systab t JOIN dbo.sysusers u ON t.creator = u.uid
          WHERE u.name = :schema_name and table_type =1
        """) #table_type <>21 #sk
        ...
        if isinstance(schema, str):
            schema = schema.encode("ascii")
        # end Py2K
        tables = connection.execute(TABLE_SQL, {'schema_name':schema}) #sk

        # To print all values from each row
        # for row in tables:
        #     print(row)  # This prints the entire row as a tuple

        return [t[0] for t in tables]
        #return [t["name"] for t in tables] #sk
